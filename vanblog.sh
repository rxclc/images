...
COMPOSE_URL="https://raw.githubusercontent.com/Mereithhh/vanblog/master/docker-compose/docker-compose-template.yml"
SCRIPT_URL="https://raw.githubusercontent.com/Mereithhh/vanblog/master/scripts/vanblog.sh"
...
install_vanblog() {
  ...
  if [[ $? != 0 ]]; then
    echo -e "正在安装 Docker"
    bash <(curl -sL https://${Get_Docker_URL}) >/dev/null 2>&1
    if [[ $? != 0 ]]; then
